...
def is_filename_correct(filename):
    # correct_pattern = 20151024192956_2.tar
    correct_pattern = re.compile('^\d{14}_\d.tar$')

    # correct_pattern = Cam_0_20150829235933_888629_TO_Cam_0_20150829235936_888801.mp4
    # correct_pattern = re.compile('^Cam_\d_\d{14}_\d{6}_TO_Cam_\d_\d{14}_\d{6}.mp4$')
    return None != re.match(correct_pattern, filename)

def get_cam(filename):
    split = re.split('_|\.', filename, 2)
    return split[1]
...
class Copier(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            # checks if the execution of cmd was successful
            if exit_status == 0:
...
class Memory(threading.Thread):
    ...
    def add_completed_files(self, completed_file):
        ...
        # Information about the absolute number of copied files
        status_shelf['progress'] += 1
        status_shelf.sync()
        logger.debug('status: ' + str(status_shelf['progress'])
                     + ' | copied : ' + completed_file)
